...
% To illustrate
% Show the graphs for the second keypoint in kp2 where sigma = 4
figure,
imshow(image,[]);
hold on
cx = 4*cell2mat(kp2{2}{1}(1));
cy = 4*cell2mat(kp2{2}{1}(2));
rd = 2*cell2mat(kp2{2}{1}(3));
...
truesize([256,256]);
hold on
cx = cell2mat(kp2{2}{1}(1));
cy = cell2mat(kp2{2}{1}(2));
rd = cell2mat(kp2{2}{1}(3));
rectangle('Position',[cx-rd,cy-rd,2*rd,2*rd],'Curvature',[1,1],'linewidth',1,'EdgeColor','g');
hold off

figure,imshow(graMag2{2,1},[]);truesize([256,256]);title('Gradient Magnitude');
figure,imshow(graDir2{2,1},[]);truesize([256,256]);title('Gradient Direction');
figure,imshow(wGraMag2{2,1},[]);truesize([256,256]);title('Weighted Gradient Magnitude');
